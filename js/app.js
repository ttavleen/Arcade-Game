...
Player.prototype.update = function(dt) {
  if (this.y <= 0) {
    ...
    alertify.alert('You win!', 'Congratulations!<br>You got char boy to the water!');
    score += 1;
    $('#score').text(score);
  }
};
...
function checkCollisions(allEnemies, player) {
  for(i = 0; i < allEnemies.length; i++) {
    if ((player.y >= allEnemies[i].y - COLLIDED && player.y <= allEnemies[i].y + COLLIDED) && (player.x >= allEnemies[i].x - COLLIDED && player.x <= allEnemies[i].x + COLLIDED)) {
      alertify.alert('Collision detected!', 'Oops, looks like char boy just hit a bug!<br>Your score is <strong>reset</strong> and you go back to the start!');
      score = 0;
      $('#score').text(score);
